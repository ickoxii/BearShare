    ...
    E: std::error::Error + Send + Sync + 'static,
{
    ...
    let client_pub_bytes: [u8; 32] = ch_payload
        .get(32..64)
        .ok_or_else(|| anyhow!("client_hello payload too short for pubkey"))?
        .try_into()
    ...
    let mut sh_payload = Vec::with_capacity(64);
    sh_payload.extend_from_slice(&server_random);
    sh_payload.extend_from_slice(server_pub.as_bytes());
